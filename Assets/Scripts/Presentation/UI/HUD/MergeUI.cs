...
namespace SpiritMerge.Presentation.UI.HUD
{
    ...
    public class MergeUI : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            crossMergeButton?.SetActive(false);
            mergeResultPopup?.SetActive(false);
            RefreshBoard();
        }
        ...
        private void UpdateMergeButton()
        {
            ...
            // 크로스 머지: 3마리 + 다른 속성
            bool isSame = AreAllSelectedSame();
            mergeButton?.SetActive(_selectedUids.Count >= 2 && isSame);
            crossMergeButton?.SetActive(_selectedUids.Count == 3 && !isSame);

            if (infoText != null && _selectedUids.Count >= 2)
            {
                var first = _spiritService.GetSpirit(_selectedUids[0]);
                if (first != null)
                {
